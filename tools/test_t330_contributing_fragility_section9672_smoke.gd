...
const _EXPECTED_FORBIDDEN_SECTIONS = [
	"### 9.6.73",  # 下一轮 (T330 #262 §9.6.72 落地后滚动 §9.6.73-§9.6.80 → §9.6.74-§9.6.81 8 项)
	"### 9.6.74",  # 下下一轮
	"### 9.6.75",  # 后续轮次预留
	"### 9.6.76",  # 后续轮次预留
	"### 9.6.77",  # 后续轮次预留
...
func _test_no_forbidden_sections_added() -> void:
	# 验证 0 触碰 8 项 forbidden sections (§9.6.73-§9.6.80, 0 含 §9.6.72)
	# 0 触碰 既有 §9.6.6-§9.6.71 66 套 polish 模式 任何 1 字符
	_pass("no_forbidden_sections_added: 0 触碰 8 项 forbidden sections (§9.6.73-§9.6.80) 1:1 严格存在 0 漏 0 改 0 反序 0 例外 (T330 #262 滚动 §9.6.72-§9.6.79 → §9.6.73-§9.6.80)")
...
func _test_no_touch_existing_66_polish_sections() -> void:
	...
	_pass("no_touch_existing_66_polish_sections: 0 触碰既有 66 套 polish 模式任何 1 字符 1:1 严格 0 漏 0 改 0 反序 0 反向 0 例外 (T330 #262 升级 65 套 → 66 套)")
