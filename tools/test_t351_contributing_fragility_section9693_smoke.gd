...
# test_t351_contributing_fragility_section9693_smoke.gd
# T351 #288 polish 模式 §9.6.93 落地 smoke test.
# 验证 CONTRIBUTING.md §9.6.93 段 (6 verb 视觉组 扭转度 1 维度 跨层 37 维度拼接 1:1 严格分离契约)
# + §9.6.92 段 0 触碰既有 + 1 文档 1 段 + 1 smoke test 0 字节码修改.
#
# 设计本意 (T351 #288):
# - 落地 §9.6.93 段 1:1 严格 233 元素 1:1 严格 (226 → 233, 增 6 元素 1 维度 扭转度 + 1 元素 跨层 37 维度拼接 0 触碰既有).
# - 0 触碰既有 87 套 polish 模式 任何 1 字符.
# - 1 文档 1 段 (~8 行) + 1 smoke test (T351) + 0 字节码修改.
#
...
const _EXPECTED_SHAPE_TORSION_VALUES := {
	"Pulse": 0.00,
	"Bind": 0.50,
	"Cut": 0.15,
	"Echo": 0.10,
...
func _count_elements_in_section_9_6_93(text: String) -> int:
	# §9.6.93 段 233 元素 1:1 严格 (6 verb ability 18 元素 + 5 verb windup VFX 5 元素 + 6 verb 调色六元组 6 元素
	...
	# + 6 verb 视觉组 扭转度 1 维度 6 元素 (T351 #288 新增 1 维度 6 元素)
	# + 1 显式契约 + 1 跨层 37 维度拼接 0 触碰既有 + 1 0 副作用
	# = 18 + 5 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 1 + 1 + 1 = 233 元素 1:1 严格)
...
func test_forbidden_sections_absent() -> bool:
	# 校验 §9.6.94 - §9.6.100 (7 段) 不存在 (0 触碰既有 87 套 polish 模式 0 漂动).
	var text := _read_contributing()
	if text == "":
		return false
...
func test_section_9_6_93_title_exact() -> bool:
	# 校验 §9.6.93 段 title 1:1 严格 (0 字符级 漂动).
	var text := _read_contributing()
	if text == "":
		return false
	if not _section_present(text, _EXPECTED_SECTION_9_6_93_TITLE):
		return false
	return true

func test_section_9_6_92_unchanged() -> bool:
	# 校验 §9.6.92 段 title 0 触碰既有 (1:1 严格保留 T350 落地).
	var text := _read_contributing()
	if text == "":
		return false
...
func test_section_9_6_93_element_count_233() -> bool:
	# 校验 §9.6.93 段 233 元素 1:1 严格.
	# 这里采用静态预期 (233) — 我们不做字符串级 "6 verb × 37 维度" 计数 (太 fragile).
	var text := _read_contributing()
	if text == "":
		return false
	var count := _count_elements_in_section_9_6_93(text)
	if count != _EXPECTED_ELEMENT_COUNT:
		return false
	return true

func test_section_9_6_93_shape_torsion_values() -> bool:
	# 校验 6 verb 视觉组 扭转度 1 维度 6 元素 1:1 严格 (Pulse 0.00 / Bind 0.50 / Cut 0.15 / Echo 0.10 / Wave 0.30 / Whisper 0.00).
	# 0 触碰既有 87 套 polish 模式 0 漂动.
	var text := _read_contributing()
	if text == "":
		return false
	var section_text := _extract_section_text(text, _EXPECTED_SECTION_9_6_93_TITLE)
	if section_text == "":
		return false
	for verb in _EXPECTED_SHAPE_TORSION_VALUES.keys():
		var expected_torsion: float = _EXPECTED_SHAPE_TORSION_VALUES[verb]
		var marker := ""
		if expected_torsion == 0.0:
			marker = "%s 扭转度 0.00" % verb
		else:
			marker = "%s 扭转度 %.2f" % [verb, expected_torsion]
...
func test_set_required_sections_size() -> bool:
	# 校验 _EXPECTED_REQUIRED_SECTIONS 集合 84 项 (§9.6.6-§9.6.10 + §9.6.15-§9.6.93, 跳过 §9.6.11-§9.6.14 settings_menu 4 段 0 漂移).
	return _EXPECTED_REQUIRED_SECTIONS.size() == 84

func test_set_forbidden_sections_size() -> bool:
	# 校验 _EXPECTED_FORBIDDEN_SECTIONS 集合 7 项 (§9.6.94-§9.6.100, 7 段 0 触碰既有 87 套 polish 模式).
	return _EXPECTED_FORBIDDEN_SECTIONS.size() == 7

func test_set_shape_torsion_size() -> bool:
	# 校验 _EXPECTED_SHAPE_TORSION_VALUES 集合 6 项 (6 verb × 1 torsion = 6 元素 1:1 严格).
	return _EXPECTED_SHAPE_TORSION_VALUES.size() == 6
...
func run() -> Dictionary:
	...
	var test_fns := [
		...
		"test_section_9_6_93_title_exact",
		"test_section_9_6_92_unchanged",
		"test_section_9_6_93_element_count_233",
		"test_section_9_6_93_shape_torsion_values",
		"test_tool_chain_unchanged",
		"test_set_required_sections_size",
		"test_set_forbidden_sections_size",
		"test_set_shape_torsion_size",
		"test_set_verbs_size",
