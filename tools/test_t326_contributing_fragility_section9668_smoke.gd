...
const _EXPECTED_FORBIDDEN_SECTIONS = [
	"### 9.6.69",  # 下一轮 (T326 #257 §9.6.68 落地后滚动 §9.6.69-§9.6.76 → §9.6.70-§9.6.77 8 项)
	"### 9.6.70",  # 下下一轮
	"### 9.6.71",  # 后续轮次预留
	"### 9.6.72",  # 后续轮次预留
	"### 9.6.73",  # 后续轮次预留
...
func _test_palette_saturation_6_verbs_1_to_1_strict() -> void:
	# 验证 6 verb 调色家族 饱和度 各自 1 饱和度 1:1 严格 跨 6 verb 0 漏 0 改 0 撞 0 共享
...
func _test_palette_midpoint_6_verbs_1_to_1_strict() -> void:
	...
	var verbs_match: bool = true
	for verb in _EXPECTED_6_VERBS:
		if verb not in verbs_in_midpoint:
			verbs_match = false
			break
	if verbs_match:
...
func _test_palette_saturation_includes_whisper() -> void:
	# 验证 6 verb 调色家族 饱和度 含 Whisper (1:1 严格 0 漏 0 改)
...
func _test_relationship_9_6_66_present() -> void:
	# 验证 关系段 与 §9.6.66 (6 verb ability + 5 verb windup VFX + 6 verb 调色六元组 + 6 verb audio 家族 1 维度 + 6 verb HUD 冷光勾边 1 维度 + 6 verb 调色家族 灰度 1 维度 + 6 verb 调色家族 亮边 1 维度 + 6 verb 调色家族 暗边 1 维度 跨层 10 维度拼接) 1:1 严格
...
func _test_no_forbidden_sections_added() -> void:
	# 验证 0 漂 0 加 §9.6.69 或后续 (T326 #257 §9.6.68 落地后滚动 §9.6.69-§9.6.76 → §9.6.70-§9.6.77 8 项)
...
func _test_6_verb_palette_saturation_6_1_to_1_strict() -> void:
	# 验证 6 verb 调色家族 饱和度 1 元素 各自 6 verb 1:1 严格 镜像
...
func _test_required_6_verb_palette_saturation_6() -> void:
	if _EXPECTED_REQUIRED_6_VERB_PALETTE_SATURATION_6:
		_pass("required_6_verb_palette_saturation_6: 6 verb 调色家族 饱和度 6 元素 1:1 严格 0 漏 0 改 0 反序")
	else:
