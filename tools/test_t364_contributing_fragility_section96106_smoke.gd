...
# 派生自 §9.6.105 intensity I (视觉强度 范数乘积) + §9.6.101 curvedness C (范数均方根), K = I × C 视觉刚度 范数乘积.
extends RefCounted
...
const _EXPECTED_FORMULA_KEY := "K = I × C"
...
func run() -> Dictionary:
	...
	if not _assert_true(text.find(_EXPECTED_ELEMENT_KEY) >= 0,
			"stiffness 键名 0 找到 (派生自 I × C koenderink 1992 + Willmore 1990 复合体系)"):
		failures += 1
	...
	if not _assert_true(text.find(_EXPECTED_FORMULA_KEY) >= 0,
			"派生公式 " + _EXPECTED_FORMULA_KEY + " 0 找到 (强度 范数乘积 × 弯曲均方根 视觉刚度范数乘积)"):
		failures += 1
	...
	if not _assert_true(text.find(_EXPECTED_EPSILON_NOTE) >= 0,
			"0 显式独立 ε 注入 0 双计数 0 三重计数 标记 0 找到 (派生自 I 单路 ε=0.05 + C 单路 ε=0.05 双路共享, 视觉平滑)"):
		failures += 1
