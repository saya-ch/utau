...
const _EXPECTED_FORMULA_KEY := "K = I × (1 - |S_I|)"
const _EXPECTED_TOTAL_ELEMENTS := "324 元素"
const _EXPECTED_CROSS_LAYER := "跨层 50 维度拼接"
const _EXPECTED_RELATION_PARENT := "与 §9.6.105 关系"
const _EXPECTED_RELATION_STEP_9 := "与 §9.6.1 9 步关系"
const _EXPECTED_NEW_6_ELEMENTS_NOTE := "T364 #304 新增 6 元素"
...
func _assert_true(cond: bool, msg: String) -> bool:
	if not cond:
		push_error("T364 §9.6.106 smoke FAILED: " + msg)
	return cond
...
func run() -> Dictionary:
	...
	if text.length() == 0:
		push_error("T364 §9.6.106 smoke FAILED: CONTRIBUTING.md 不可读")
	...
	if not _assert_true(text.find(_EXPECTED_ELEMENT_KEY) >= 0,
			"stiffness 键名 0 找到 (派生自 I × (1 - |S_I|) koenderink 1992 + Willmore 1990 复合体系)"):
		failures += 1
	...
	# 断言 9: 派生公式 K = I × (1 - |S_I|) 存在
	if not _assert_true(text.find(_EXPECTED_FORMULA_KEY) >= 0,
			"派生公式 " + _EXPECTED_FORMULA_KEY + " 0 找到 (I 视觉强度 × (1 - |S_I|) 视觉刚度 强度与形状偏离乘积)"):
		failures += 1

	# 断言 10: 总元素 324
	if not _assert_true(text.find(_EXPECTED_TOTAL_ELEMENTS) >= 0,
			"总元素 324 0 找到 (317 §9.6.105 元素 + 6 刚度 + 1 跨层 50 维度拼接 0 触碰既有)"):
		failures += 1

	# 断言 11: 跨层 50 维度拼接 存在
	if not _assert_true(text.find(_EXPECTED_CROSS_LAYER) >= 0,
			"跨层 50 维度拼接 0 找到 (从 §9.6.105 跨层 49 升级 1 维 → 跨层 50)"):
		failures += 1

	# 断言 12: 与 §9.6.105 关系段 存在
	if not _assert_true(text.find(_EXPECTED_RELATION_PARENT) >= 0,
			"与 §9.6.105 关系段 0 找到 (姊妹段 + 派生段 契约)"):
		failures += 1
	...
	# 断言 14: T364 #304 新增 6 元素 标记 存在
	if not _assert_true(text.find(_EXPECTED_NEW_6_ELEMENTS_NOTE) >= 0,
			"T364 #304 新增 6 元素 标记 0 找到 (T364 #304 跨 1 任务 1 轮落地)"):
		failures += 1

	# 断言 15: 0 显式独立 ε 注入 0 双计数 0 三重计数 标记 存在
	if not _assert_true(text.find(_EXPECTED_EPSILON_NOTE) >= 0,
			"0 显式独立 ε 注入 0 双计数 0 三重计数 标记 0 找到 (派生自 I 单路 ε=0.05 + S_I 单路 ε=0.05 双路共享, 视觉平滑)"):
		failures += 1

	var issues: Array = []
	if failures > 0:
		issues.append("%d 断言失败" % failures)
		print("T364 §9.6.106 smoke FAILED: %d 断言失败" % failures)
	else:
		print("T364 §9.6.106 smoke PASSED (15 断言 0 失败): 324 元素 / 跨层 50 维度 / 6 元素 1:1 严格")
