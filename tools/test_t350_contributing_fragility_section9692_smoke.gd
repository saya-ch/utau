...
func test_forbidden_sections_absent() -> bool:
	# 校验 §9.6.93 - §9.6.100 (8 段) 不存在 (0 触碰既有 86 套 polish 模式 0 漂动).
	var text := _read_contributing()
	if text == "":
		return false
...
func test_section_9_6_92_shape_helicity_values() -> bool:
	# 校验 6 verb 视觉组 螺旋度 1 维度 6 元素 1:1 严格 (Pulse 0.00 / Bind 0.80 / Cut 0.30 / Echo 0.20 / Wave 0.50 / Whisper 0.00).
...
func test_set_forbidden_sections_size() -> bool:
	# 校验 _EXPECTED_FORBIDDEN_SECTIONS 集合 8 项 (§9.6.93-§9.6.100, 8 段 0 触碰既有 86 套 polish 模式).
