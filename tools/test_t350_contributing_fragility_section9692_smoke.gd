...
# test_t350_contributing_fragility_section9692_smoke.gd
# T350 #287 polish 模式 §9.6.92 落地 smoke test.
# 验证 CONTRIBUTING.md §9.6.92 段 (6 verb 视觉组 螺旋度 1 维度 跨层 36 维度拼接 1:1 严格分离契约)
# + §9.6.91 段 0 触碰既有 + 1 文档 1 段 + 1 smoke test 0 字节码修改.
#
# 设计本意 (T350 #287):
# - 落地 §9.6.92 段 1:1 严格 226 元素 1:1 严格 (219 → 226, 增 6 元素 1 维度 螺旋度 + 1 元素 跨层 36 维度拼接 0 触碰既有).
# - 0 触碰既有 86 套 polish 模式 任何 1 字符.
# - 1 文档 1 段 (~8 行) + 1 smoke test (T350) + 0 字节码修改.
#
...
func _count_elements_in_section_9_6_92(text: String) -> int:
	# §9.6.92 段 226 元素 1:1 严格 (6 verb ability 18 元素 + 5 verb windup VFX 5 元素 + 6 verb 调色六元组 6 元素
	...
	# + 6 verb 视觉组 螺旋度 1 维度 6 元素 (T350 #287 新增 1 维度 6 元素)
	# + 1 显式契约 + 1 跨层 36 维度拼接 0 触碰既有 + 1 0 副作用
	# = 18 + 5 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 1 + 1 + 1 = 226 元素 1:1 严格)
...
func test_forbidden_sections_absent() -> bool:
	# 校验 §9.6.94 - §9.6.100 (7 段) 不存在 (0 触碰既有 87 套 polish 模式 0 漂动, T351 #288 落地后 §9.6.93 段从 forbidden 移除).
	var text := _read_contributing()
	if text == "":
		return false
...
func test_section_9_6_92_title_exact() -> bool:
	# 校验 §9.6.92 段 title 1:1 严格 (0 字符级 漂动).
	var text := _read_contributing()
	if text == "":
		return false
	if not _section_present(text, _EXPECTED_SECTION_9_6_92_TITLE):
		return false
	return true

func test_section_9_6_91_unchanged() -> bool:
	# 校验 §9.6.91 段 title 0 触碰既有 (1:1 严格保留 T349 落地).
	var text := _read_contributing()
	if text == "":
		return false
	if not _section_present(text, _EXPECTED_SECTION_9_6_91_TITLE):
		return false
	return true

func test_section_9_6_92_element_count_226() -> bool:
	# 校验 §9.6.92 段 226 元素 1:1 严格.
	# 这里采用静态预期 (226) — 我们不做字符串级 "6 verb × 36 维度" 计数 (太 fragile).
	var text := _read_contributing()
	if text == "":
		return false
	var count := _count_elements_in_section_9_6_92(text)
	if count != _EXPECTED_ELEMENT_COUNT:
		return false
	return true

func test_section_9_6_92_shape_helicity_values() -> bool:
	# 校验 6 verb 视觉组 螺旋度 1 维度 6 元素 1:1 严格 (Pulse 0.00 / Bind 0.40 / Cut 0.10 / Echo 0.05 / Wave 0.20 / Whisper 0.00).
	# 0 触碰既有 86 套 polish 模式 0 漂动.
	var text := _read_contributing()
	if text == "":
		return false
	var section_text := _extract_section_text(text, _EXPECTED_SECTION_9_6_92_TITLE)
	if section_text == "":
		return false
	for verb in _EXPECTED_SHAPE_HELICITY_VALUES.keys():
		var expected_helicity: float = _EXPECTED_SHAPE_HELICITY_VALUES[verb]
		var marker := ""
		if expected_helicity == 0.0:
			marker = "%s 螺旋度 0.00" % verb
		else:
			marker = "%s 螺旋度 %.2f" % [verb, expected_helicity]
...
func test_set_required_sections_size() -> bool:
	# 校验 _EXPECTED_REQUIRED_SECTIONS 集合 83 项 (§9.6.6-§9.6.10 + §9.6.15-§9.6.92, 跳过 §9.6.11-§9.6.14 settings_menu 4 段 0 漂移).
	return _EXPECTED_REQUIRED_SECTIONS.size() == 83

func test_set_forbidden_sections_size() -> bool:
	# 校验 _EXPECTED_FORBIDDEN_SECTIONS 集合 7 项 (§9.6.94-§9.6.100, 7 段 0 触碰既有 87 套 polish 模式, T351 #288 滚动后).
	return _EXPECTED_FORBIDDEN_SECTIONS.size() == 7

func test_set_shape_helicity_size() -> bool:
	# 校验 _EXPECTED_SHAPE_HELICITY_VALUES 集合 6 项 (6 verb × 1 helicity = 6 元素 1:1 严格).
	return _EXPECTED_SHAPE_HELICITY_VALUES.size() == 6
...
func run() -> Dictionary:
	...
	var test_fns := [
		...
		"test_section_9_6_92_title_exact",
		"test_section_9_6_91_unchanged",
		"test_section_9_6_92_element_count_226",
		"test_section_9_6_92_shape_helicity_values",
		"test_tool_chain_unchanged",
		"test_set_required_sections_size",
		"test_set_forbidden_sections_size",
		"test_set_shape_helicity_size",
		"test_set_verbs_size",
