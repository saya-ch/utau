...
# test_t349_contributing_fragility_section9691_smoke.gd
# T349 #286 polish 模式 §9.6.91 落地 smoke test.
# 验证 CONTRIBUTING.md §9.6.91 段 (6 verb 视觉组 切变 1 维度 跨层 35 维度拼接 1:1 严格分离契约)
# + §9.6.90 段 0 触碰既有 + 1 文档 1 段 + 1 smoke test 0 字节码修改.
#
# 设计本意 (T349 #286):
# - 落地 §9.6.91 段 1:1 严格 219 元素 1:1 严格 (212 → 219, 增 6 元素 1 维度 切变 + 1 元素 跨层 35 维度拼接 0 触碰既有).
# - 0 触碰既有 85 套 polish 模式 任何 1 字符.
# - 1 文档 1 段 (~8 行) + 1 smoke test (T349) + 0 字节码修改.
#
...
const _EXPECTED_FORBIDDEN_SECTIONS: Array = [
	"### 9.6.96",
	"### 9.6.97",
	"### 9.6.99",
	"### 9.6.100",
...
const _EXPECTED_ELEMENT_COUNT := 219
...
const _EXPECTED_SHAPE_SHEAR_VALUES := {
	"Pulse": 0.00,
	"Bind": 0.30,
	"Cut": 0.50,
	"Echo": 0.10,
...
func _count_elements_in_section_9_6_91(text: String) -> int:
	# §9.6.91 段 219 元素 1:1 严格 (6 verb ability 18 元素 + 5 verb windup VFX 5 元素 + 6 verb 调色六元组 6 元素
	...
	# + 6 verb 视觉组 切变 1 维度 6 元素 (T349 #286 新增 1 维度 6 元素)
	# + 1 显式契约 + 1 跨层 35 维度拼接 0 触碰既有 + 1 0 副作用
	# = 18 + 5 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 1 + 1 + 1 = 219 元素 1:1 严格)
...
func test_section_9_6_91_title_exact() -> bool:
	# 校验 §9.6.91 段 title 1:1 严格 (0 字符级 漂动).
	var text := _read_contributing()
	if text == "":
		return false
...
func test_section_9_6_90_unchanged() -> bool:
	# 校验 §9.6.90 段 title 0 触碰既有 (1:1 严格保留 T348 落地).
	var text := _read_contributing()
	if text == "":
		return false
...
func test_section_9_6_91_element_count_219() -> bool:
	# 校验 §9.6.91 段 219 元素 1:1 严格.
	# 这里采用静态预期 (219) — 我们不做字符串级 "6 verb × 35 维度" 计数 (太 fragile).
	var text := _read_contributing()
	if text == "":
		return false
	var count := _count_elements_in_section_9_6_91(text)
	if count != _EXPECTED_ELEMENT_COUNT:
		return false
	return true

func test_section_9_6_91_shape_shear_values() -> bool:
	# 校验 6 verb 视觉组 切变 1 维度 6 元素 1:1 严格 (Pulse 0.00 / Bind 0.30 / Cut 0.50 / Echo 0.10 / Wave 0.20 / Whisper 0.00).
	# 0 触碰既有 85 套 polish 模式 0 漂动.
	var text := _read_contributing()
	if text == "":
		return false
	var section_text := _extract_section_text(text, _EXPECTED_SECTION_9_6_91_TITLE)
	if section_text == "":
		return false
	for verb in _EXPECTED_SHAPE_SHEAR_VALUES.keys():
		var expected_shear: float = _EXPECTED_SHAPE_SHEAR_VALUES[verb]
		var marker := ""
		if expected_shear == 0.0:
			marker = "%s 切变 0.00" % verb
		else:
			marker = "%s 切变 %.2f" % [verb, expected_shear]
...
func test_set_required_sections_size() -> bool:
	# 校验 _EXPECTED_REQUIRED_SECTIONS 集合 82 项 (§9.6.6-§9.6.10 + §9.6.15-§9.6.91, 跳过 §9.6.11-§9.6.14 settings_menu 4 段 0 漂移).
	return _EXPECTED_REQUIRED_SECTIONS.size() == 82
...
func test_set_shape_shear_size() -> bool:
	# 校验 _EXPECTED_SHAPE_SHEAR_VALUES 集合 6 项 (6 verb × 1 shear = 6 元素 1:1 严格).
	return _EXPECTED_SHAPE_SHEAR_VALUES.size() == 6
...
func run() -> Dictionary:
	...
	var test_fns := [
		...
		"test_section_9_6_91_title_exact",
		"test_section_9_6_90_unchanged",
		"test_section_9_6_91_element_count_219",
		"test_section_9_6_91_shape_shear_values",
		"test_tool_chain_unchanged",
		"test_set_required_sections_size",
		"test_set_forbidden_sections_size",
		"test_set_shape_shear_size",
		"test_set_verbs_size",
