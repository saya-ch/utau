...
func _count_elements_in_section_9_6_77(text: String) -> int:
	...
	#
	# 通过 §9.6.77 段 段落正文 计数 "6 verb" / "5 verb" / "1 显式契约" / "1 跨层 21 维度拼接" / "1 0 副作用" 出现次数.
	var idx := text.find(_EXPECTED_SECTION_9_6_77_TITLE)
	if idx == -1:
		return -1
	# 找 §9.6.77 段 结束位置 (下一个 --- 或 ## 段).
	...
	# 元素计数: §9.6.77 段 128 元素 1:1 严格 (静态预期) — 我们不做字符串级 "6 verb × 19 维度" 计数 (太 fragile),
...
func test_section_9_6_77_shape_rotation_values() -> bool:
	...
	# Pulse 0.00 / Bind 60.00 / Cut 0.00 / Echo 30.00 / Wave 0.00 / Whisper 0.00.
	var text := _read_contributing()
	if text == "":
		return false
	...
	for verb in _EXPECTED_VERBS:
		var expected := _EXPECTED_SHAPE_ROTATION_VALUES[verb]
		# 段 文字 内 必须 包含 "verb 旋转 <value>" pattern.
		var needle := verb + " 旋转 " + str(expected)
...
func test_tool_chain_unchanged() -> bool:
	...
	# 通过 校验 工具链 文件 存在 校验 0 触碰 (T335 仅新增 T335 smoke test 1 个 + 滚动 test_t334 / test_t333 _EXPECTED_FORBIDDEN_SECTIONS 1 段).
...
func test_set_required_sections_size() -> bool:
	# 校验 _EXPECTED_REQUIRED_SECTIONS 集合 72 项 (§9.6.6-§9.6.77, 72 段).
