...
const _EXPECTED_FORMULA_KEY := "B = 1 − T_u"
const _EXPECTED_TOTAL_ELEMENTS := "345 元素"
const _EXPECTED_CROSS_LAYER := "跨层 53 维度拼接"
const _EXPECTED_RELATION_PARENT := "与 §9.6.108 关系"
const _EXPECTED_RELATION_STEP_9 := "与 §9.6.1 9 步关系"
const _EXPECTED_NEW_6_ELEMENTS_NOTE := "T367 #308 新增 6 元素"
const _EXPECTED_EPSILON_NOTE := "视觉平滑 由 T_u 单路 0 显式独立 ε 注入"
...
func _assert_true(cond: bool, msg: String) -> bool:
	if not cond:
		push_error("T367 §9.6.109 smoke FAILED: " + msg)
	return cond
...
func run() -> Dictionary:
	...
	if text.length() == 0:
		push_error("T367 §9.6.109 smoke FAILED: CONTRIBUTING.md 不可读")
	...
	# 断言 2: brittleness 键名存在
	if not _assert_true(text.find(_EXPECTED_ELEMENT_KEY) >= 0,
			"brittleness 键名 0 找到 (派生自 T_u 视觉韧度范数乘积 反演 B = 1 − T_u)"):
		failures += 1
	...
	# 断言 9: 派生公式 B = 1 − T_u 存在
	if not _assert_true(text.find(_EXPECTED_FORMULA_KEY) >= 0,
			"派生公式 " + _EXPECTED_FORMULA_KEY + " 0 找到 (视觉脆度归一化减法, Griffith 1921 反演)"):
		failures += 1

	# 断言 10: 总元素 345
	if not _assert_true(text.find(_EXPECTED_TOTAL_ELEMENTS) >= 0,
			"总元素 345 0 找到 (338 §9.6.108 元素 + 6 脆度 + 1 跨层 53 维度拼接 0 触碰既有)"):
		failures += 1

	# 断言 11: 跨层 53 维度拼接 存在
	if not _assert_true(text.find(_EXPECTED_CROSS_LAYER) >= 0,
			"跨层 53 维度拼接 0 找到 (从 §9.6.108 跨层 52 升级 1 维 → 跨层 53)"):
		failures += 1

	# 断言 12: 与 §9.6.108 关系段 存在
	if not _assert_true(text.find(_EXPECTED_RELATION_PARENT) >= 0,
			"与 §9.6.108 关系段 0 找到 (姊妹段 + 反演段 契约)"):
		failures += 1
	...
	# 断言 14: T367 #308 新增 6 元素 标记 存在
	if not _assert_true(text.find(_EXPECTED_NEW_6_ELEMENTS_NOTE) >= 0,
			"T367 #308 新增 6 元素 标记 0 找到 (T367 #308 跨 1 任务 1 轮落地)"):
		failures += 1

	# 断言 15: 0 显式独立 ε 注入 标记 存在 (B = 1 − T_u 减法 0 引入新 ε, 与 T_u 单路共享)
	if not _assert_true(text.find(_EXPECTED_EPSILON_NOTE) >= 0,
			"0 显式独立 ε 注入 标记 0 找到 (与 T_u 单路共享 ε=0.05 视觉平滑, 0 双计数 0 三重计数)"):
		failures += 1

	var issues: Array = []
	if failures > 0:
		issues.append("%d 断言失败" % failures)
		print("T367 §9.6.109 smoke FAILED: %d 断言失败" % failures)
	else:
		print("T367 §9.6.109 smoke PASSED (15 断言 0 失败): 345 元素 / 跨层 53 维度 / 6 元素 1:1 严格")
