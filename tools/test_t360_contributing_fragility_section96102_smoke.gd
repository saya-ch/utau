...
const _EXPECTED_FORMULA_KEY := "S = (2/π) * arctan((κ₁+κ₂)/(κ₁-κ₂))"
const _EXPECTED_TOTAL_ELEMENTS := "296 元素"
const _EXPECTED_CROSS_LAYER := "跨层 46 维度拼接"
const _EXPECTED_RELATION_PARENT := "与 §9.6.101 关系"
const _EXPECTED_RELATION_STEP_9 := "与 §9.6.1 9 步关系"
const _EXPECTED_NEW_6_ELEMENTS_NOTE := "T360 #299 新增 6 元素"
...
func _assert_true(cond: bool, msg: String) -> bool:
	if not cond:
		push_error("T360 §9.6.102 smoke FAILED: " + msg)
	return cond
...
func run() -> int:
	...
	if text.length() == 0:
		push_error("T360 §9.6.102 smoke FAILED: CONTRIBUTING.md 不可读")
		return 1
	...
	# 断言 2: shape_index 键名存在
	if not _assert_true(text.find(_EXPECTED_ELEMENT_KEY) >= 0,
			"shape_index 键名 0 找到 (派生自 κ₁+κ₂ 角度投影 koenderink 1992 shape index 体系)"):
		failures += 1
	...
	# 断言 9: 派生公式 S = (2/π) * arctan((κ₁+κ₂)/(κ₁-κ₂)) 存在
	if not _assert_true(text.find(_EXPECTED_FORMULA_KEY) >= 0,
			"派生公式 " + _EXPECTED_FORMULA_KEY + " 0 找到 (koenderink 1992 shape index 体系 角度投影)"):
		failures += 1

	# 断言 10: 总元素 296
	if not _assert_true(text.find(_EXPECTED_TOTAL_ELEMENTS) >= 0,
			"总元素 296 0 找到 (289 派生 + 6 形状指数 + 1 跨层 46 维度拼接 0 触碰既有)"):
		failures += 1

	# 断言 11: 跨层 46 维度拼接 存在
	if not _assert_true(text.find(_EXPECTED_CROSS_LAYER) >= 0,
			"跨层 46 维度拼接 0 找到 (从 §9.6.101 跨层 45 升级 1 维 → 跨层 46)"):
		failures += 1

	# 断言 12: 与 §9.6.101 关系段 存在
	if not _assert_true(text.find(_EXPECTED_RELATION_PARENT) >= 0,
			"与 §9.6.101 关系段 0 找到 (姊妹段 + 派生段 互补契约)"):
		failures += 1
	...
	# 断言 14: T360 #299 新增 6 元素 标记 存在
	if not _assert_true(text.find(_EXPECTED_NEW_6_ELEMENTS_NOTE) >= 0,
			"T360 #299 新增 6 元素 标记 0 找到 (T360 #299 跨 1 任务 1 轮落地)"):
		failures += 1
	...
	if failures == 0:
		print("T360 §9.6.102 smoke PASSED (15 断言 0 失败): 296 元素 / 跨层 46 维度 / 6 元素 1:1 严格")
		return 0
	push_error("T360 §9.6.102 smoke FAILED: %d 断言失败" % failures)
	return 1
