...
const _EXPECTED_FORMULA_KEY := "E_d = H² / H_max²"
const _EXPECTED_TOTAL_ELEMENTS := "310 元素"
const _EXPECTED_CROSS_LAYER := "跨层 48 维度拼接"
const _EXPECTED_RELATION_PARENT := "与 §9.6.103 关系"
const _EXPECTED_RELATION_STEP_9 := "与 §9.6.1 9 步关系"
const _EXPECTED_NEW_6_ELEMENTS_NOTE := "T362 #302 新增 6 元素"
...
func _assert_true(cond: bool, msg: String) -> bool:
	if not cond:
		push_error("T362 §9.6.104 smoke FAILED: " + msg)
	return cond
...
func run() -> Dictionary:
	...
	if text.length() == 0:
		push_error("T362 §9.6.104 smoke FAILED: CONTRIBUTING.md 不可读")
	...
	# 断言 2: energy_density 键名存在
	if not _assert_true(text.find(_EXPECTED_ELEMENT_KEY) >= 0,
			"energy_density 键名 0 找到 (派生自 H²/H_max² Willmore 1990 局部能密度体系)"):
		failures += 1
	...
	# 断言 9: 派生公式 E_d = H² / H_max² 存在
	if not _assert_true(text.find(_EXPECTED_FORMULA_KEY) >= 0,
			"派生公式 " + _EXPECTED_FORMULA_KEY + " 0 找到 (Willmore 1990 局部能密度 H² max 归一)"):
		failures += 1

	# 断言 10: 总元素 310
	if not _assert_true(text.find(_EXPECTED_TOTAL_ELEMENTS) >= 0,
			"总元素 310 0 找到 (303 派生 + 6 能密度 + 1 跨层 48 维度拼接 0 触碰既有)"):
		failures += 1

	# 断言 11: 跨层 48 维度拼接 存在
	if not _assert_true(text.find(_EXPECTED_CROSS_LAYER) >= 0,
			"跨层 48 维度拼接 0 找到 (从 §9.6.103 跨层 47 升级 1 维 → 跨层 48)"):
		failures += 1

	# 断言 12: 与 §9.6.103 关系段 存在
	if not _assert_true(text.find(_EXPECTED_RELATION_PARENT) >= 0,
			"与 §9.6.103 关系段 0 找到 (姊妹段 + 派生段 契约)"):
		failures += 1
	...
	# 断言 14: T362 #302 新增 6 元素 标记 存在
	if not _assert_true(text.find(_EXPECTED_NEW_6_ELEMENTS_NOTE) >= 0,
			"T362 #302 新增 6 元素 标记 0 找到 (T362 #302 跨 1 任务 1 轮落地)"):
		failures += 1

	# 断言 15: 0 显式独立 ε 注入 0 双计数 0 三重计数 标记 存在
	if not _assert_true(text.find(_EXPECTED_EPSILON_NOTE) >= 0,
			"0 显式独立 ε 注入 0 双计数 0 三重计数 标记 0 找到 (派生自 H² 单路 ε=0.05 共享, 视觉平滑)"):
		failures += 1

	var issues: Array = []
	if failures > 0:
		issues.append("%d 断言失败" % failures)
		print("T362 §9.6.104 smoke FAILED: %d 断言失败" % failures)
	else:
		print("T362 §9.6.104 smoke PASSED (15 断言 0 失败): 310 元素 / 跨层 48 维度 / 6 元素 1:1 严格")
