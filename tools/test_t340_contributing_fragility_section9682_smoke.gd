...
# test_t340_contributing_fragility_section9682_smoke.gd
# T340 #274 polish 模式 §9.6.82 落地 smoke test.
# 验证 CONTRIBUTING.md §9.6.82 段 (6 verb 视觉组 减速度 1 维度 跨层 26 维度拼接 1:1 严格分离契约)
# + §9.6.81 段 0 触碰既有 + 1 文档 1 段 + 1 smoke test 0 字节码修改.
#
# 设计本意 (T340 #274):
# - 落地 §9.6.82 段 1:1 严格 158 元素 1:1 严格 (152 → 158, 增 6 元素 1 维度 减速度 + 1 元素 跨层 26 维度拼接 0 触碰既有).
# - 0 触碰既有 76 套 polish 模式 任何 1 字符.
# - 1 文档 1 段 (~30 行) + 1 smoke test (T340) + 0 字节码修改.
#
...
const _EXPECTED_SHAPE_DECELERATION_VALUES := {
	"Pulse": 0.30,
	"Bind": 0.40,
	"Cut": 0.60,
	"Echo": 0.20,
...
func _count_elements_in_section_9_6_82(text: String) -> int:
	# §9.6.82 段 158 元素 1:1 严格 (6 verb ability 18 元素 + 5 verb windup VFX 5 元素 + 6 verb 调色六元组 6 元素
	...
	# + 1 显式契约 + 1 跨层 26 维度拼接 0 触碰既有 + 1 0 副作用
	# = 18 + 5 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 6 + 1 + 1 + 1 = 158 元素 1:1 严格)
...
func test_section_9_6_82_title_exact() -> bool:
	# 校验 §9.6.82 段 title 1:1 严格 (0 字符级 漂动).
	var text := _read_contributing()
	if text == "":
		return false
...
func test_section_9_6_81_unchanged() -> bool:
	# 校验 §9.6.81 段 title 0 触碰既有 (1:1 严格保留 T339 落地).
	var text := _read_contributing()
	if text == "":
		return false
...
func test_section_9_6_82_element_count_158() -> bool:
	# 校验 §9.6.82 段 158 元素 1:1 严格.
	# 这里采用静态预期 (158) — 我们不做字符串级 "6 verb × 26 维度" 计数 (太 fragile).
	var text := _read_contributing()
	if text == "":
		return false
	var count := _count_elements_in_section_9_6_82(text)
	if count != _EXPECTED_ELEMENT_COUNT:
		return false
	return true

func test_section_9_6_82_shape_deceleration_values() -> bool:
	# 校验 6 verb 视觉组 减速度 1 维度 6 元素 1:1 严格 (Pulse 0.30 / Bind 0.40 / Cut 0.60 / Echo 0.20 / Wave 0.50 / Whisper 0.05).
	# 0 触碰既有 76 套 polish 模式 0 漂动.
	var text := _read_contributing()
	if text == "":
		return false
	var section_text := _extract_section_text(text, _EXPECTED_SECTION_9_6_82_TITLE)
	if section_text == "":
		return false
	for verb in _EXPECTED_SHAPE_DECELERATION_VALUES.keys():
		var expected_deceleration: float = _EXPECTED_SHAPE_DECELERATION_VALUES[verb]
		var marker := "%s 减速度 %.2f" % [verb, expected_deceleration]
...
func test_set_shape_deceleration_size() -> bool:
	# 校验 _EXPECTED_SHAPE_DECELERATION_VALUES 集合 6 项 (6 verb × 1 deceleration = 6 元素 1:1 严格).
	return _EXPECTED_SHAPE_DECELERATION_VALUES.size() == 6
...
func run() -> Dictionary:
	...
	var test_fns := [
		...
		"test_section_9_6_82_title_exact",
		"test_section_9_6_81_unchanged",
		"test_section_9_6_82_element_count_158",
		"test_section_9_6_82_shape_deceleration_values",
		"test_tool_chain_unchanged",
		"test_set_required_sections_size",
		"test_set_forbidden_sections_size",
		"test_set_shape_deceleration_size",
		"test_set_verbs_size",
...
func run_all_tests() -> Dictionary:
	...
	results["test_section_9_6_82_title_exact"] = test_section_9_6_82_title_exact()
	results["test_section_9_6_81_unchanged"] = test_section_9_6_81_unchanged()
	results["test_section_9_6_82_element_count_158"] = test_section_9_6_82_element_count_158()
	results["test_section_9_6_82_shape_deceleration_values"] = test_section_9_6_82_shape_deceleration_values()
	...
	results["test_set_shape_deceleration_size"] = test_set_shape_deceleration_size()
...
func get_pass_message() -> String:
	return "T340 #274 §9.6.82 6 verb 视觉组 减速度 1 维度 跨层 26 维度拼接 — 12 套 all green"
