...
### 美术规格（直接影响 game-asset-design 参数）
- **像素网格**：16×16（复古）或 32×32（现代像素），选定后全局统一。角色 32px 高→512px 画布(bottom-center)，道具 16px→256px 画布(center)
- **色板限制**：主色 4~6 个 + 辅色 3~4 个 + 强调色 1~2 个。从 STYLE_GUIDE 继承，不随意引入新颜色
- **动画帧数**：idle 4帧、walk 8帧、attack 6帧。不追求过度流畅，独立游戏味在于「读得懂」而非「播得顺」
- **UI 层级**：HUD 在上、对话框在下、菜单全屏叠加。字体统一、按钮风格统一、不混搭 UI 范式
- **负空间**：角色占画布 80~85%，留 15~20% 呼吸空间。不把角色塞满画布

### 关卡设计基础
- **引导优于说明**：用光线、敌人位置、地形暗示引导玩家，不依赖文字说明
- **安全区→挑战区→安全区**：每段高强度战斗后给喘息空间，节奏张弛有度
- **教学即关卡**：第一个场景学会移动、第二个学会跳跃、第三个学会攻击。不做单独的「教程关」
- **可重入性**：关键路径上放置回头才能到达的区域（捷径/隐藏），让玩家觉得「这游戏有东西」

### 玩家体验
- **前 5 分钟**：玩家必须完成一次核心循环（看到敌人→打败敌人→获得奖励）。5 分钟内没体验到核心循环→流失
- **难度曲线**：前 30 分钟平稳上升，30 分钟后引入转折（新敌人类型/新机制/环境变化）。不突然拔高难度
- **奖励节奏**：每 3~5 分钟一个小奖励（金币/道具/新能力预览），每 20~30 分钟一个大奖励（新区域/新机制/剧情推进）
- **死亡友好**：死亡惩罚轻（重生点近、不掉进度），鼓励尝试而非惩罚失败。Roguelike 除外

### 技术基准
- **目标分辨率**：960×540（16:9 像素游戏标准），整数缩放到 1920×1080
- **帧率**：60fps 稳定，不因粒子效果/大量敌人掉帧
- **输入响应**：按键→画面反应 < 50ms。手感第一、视觉第二
- **包体控制**：首次迭代后 < 50MB。大素材（背景）用 WebP，小素材（精灵）用 PNG
