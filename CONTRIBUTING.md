...
### 9.6.19 `_verb_windup_vfx_base.gd` 共享契约 1:1 严格分离 polish 模式 (T166 #85 + T167 #86 + T168 #86 + T169 #87 + T171 #89 + T173 #92 + T174 #93 + T174.B #94 + F013.E #159 跨 9 任务 ~100 轮落地) 文档化

> T166 (#85) + T167 (#86) + T168 (#86) + T169 (#87) + T171 (#89) + T173 (#92) + T174 (#93) + T174.B (#94) + F013.E (#159) 跨 9 任务在 `src/scripts/_verb_windup_vfx_base.gd` (`class_name VerbWindupVFXBase extends Node2D`) 落地 5 verb windup VFX (PulseWindupVFX / BindWindupVFX / CutWindupVFX / EchoWindupVFX / WaveWindupVFX) 共享契约 (3 共享字段 + 1 `_ready()` z_index 集中 + 1 `_process(delta)` 生命周期 + 1 `_activate_windup_tween()` ramp-in tween + 1 `fade_out_and_free()` 0.05s 退出 = 7 件) 1:1 严格分离. 1 verb windup VFX (WhisperWindupVFX, F013.E #159 落地) **当前 0 extend** base (`extends Node2D` 自实现 `_lifetime` / `_max_lifetime` / `_is_active` (注意: 字段名 `_is_active` vs base `_active` 命名偏离) + 自实现 `_process(delta)` + 自实现 `fade_out_and_free()`) — T274 (#193) 落地后**下一轮 T275+ 重构目标** (与 §9.6.18 `_verb_ability_base.gd` 5 verb 0 重写 4 verb + 1 verb 显式重写 模式 镜像: 5 verb 0 extend base 是 1:1 严格分离基础, 1 verb 0 extend 是已知 drift 需下一轮 polish 收回). 任何"加新 1 verb"或"重构共享契约"时**必须**严格按 1:1 共享契约复制既有模式 (0 漏 0 改 1 字符). 跳出 §9.6.18 6 verb ability 共享契约 范畴 记录 **windup VFX 类层级共享契约** polish 模式 (T174.B #94 byte-identical `fade_out_and_free()` 5 verb 落地后, F013.E #159 第 6 verb 时 0 走该 base 1:1, 文档化该 drift 防 polish 期重踩 "加新 1 verb 共享字段 0 复制 1:1 / 6 verb windup 各自 byte-identical copy 漂移 / Whisper 自实现 vs 5 verb 共享 base 漂移" 类 pre-existing 风险).
...
3. **修复** — 严格按 7 件共享契约 1:1 复制既有模式 (`_verb_windup_vfx_base.gd` 是 canonical 副本, 5 verb 各自 extends 继承 0 重写, 1 verb Whisper **当前 0 extend** 是已知 drift):
   - **3 共享字段** 0 重声明 — `_max_lifetime: float = 0.10` (T166 #85 5 verb 默认 windup_time, 0.04~0.10s 区间, tween duration / safety net 双重数据源) + `_lifetime: float = 0.0` (T166 #85 _process 累加, 跨 frame monotonic 计数) + `_active: bool = false` (T166 #85 _process 早退守卫 + fade_out_and_free 0 重入守卫). 5 verb 子类**必须** 0 重声明 (会 shadow base 字段破坏 lifecycle contract). 任何"加新 1 verb 共享字段"必须严格在 base 加 1 个字段 + 5 verb 各自 0 改 0 重声明 0 shadow. **Whisper 0 extend base 漂移** — `whisper_windup_vfx.gd` 字段名 `_is_active` vs base `_active` 命名偏离 + `_max_lifetime = 0.10` 默认值与 base 同步但独立声明 → 任何"加新 1 verb"必须**先**在 base 加 1 个字段, **再**5 verb 0 触碰, 6 verb (Whisper) T275+ 重构目标.
   - **1 `_ready()` z_index 集中** 0 重写 — `z_index = 10` (5 verb windup VFX 都渲染在世界 (player/sprites) 之上 HUD 之下, 集中 base 避免 5 verb 各自漏设). 5 verb 子类**必须** 0 override `_ready()`. 任何"加新 1 verb" 0 触碰.
   - **1 `_process(delta)` 生命周期 0 重写** — 5 verb 各自 `_process` 是 byte-identical copy (T166 #85 + T167 #86 + T168 #86 + T169 #87 + T171 #89 落地时各写一遍), 后由 base 1:1 集中: `if not _active: return` 守卫 + `_lifetime += delta` 累加 + `if _lifetime >= _max_lifetime: _active = false; queue_free(); return` safety net + `queue_redraw()` 跨 frame 触发 `_draw()`. 5 verb 子类**必须** 0 override `_process()`. **Whisper 0 extend base 漂移** — `whisper_windup_vfx.gd _process` 自实现 `if _lifetime >= _max_lifetime: fade_out_and_free()` (调 base 同名方法, 但 base 同名方法 `_active` 字段名) — Whisper 自实现 0 触碰 base, 6 verb (Whisper) T275+ 重构目标.
   - **1 `_activate_windup_tween()` ramp-in tween (T174 #93 落地) 0 重写** — `modulate.a = 0.0` 初始 alpha + `create_tween()` + `set_trans(Tween.TRANS_QUAD)` + `set_ease(Tween.EASE_OUT)` + `tween_property(self, "modulate:a", 1.0, _max_lifetime)` (modulate:a 0.0→1.0 over _max_lifetime, TRANS_QUAD EASE_OUT front-loads 可见性) + `queue_redraw()`. 5 verb `trigger()` 末尾必须 `_activate_windup_tween()` (T174.B #94 替代 T174 #93 之前 5 verb 各自 4-line tween block). 5 verb 子类**必须** 0 override `_activate_windup_tween()`. 任何"加新 1 verb" 0 触碰 base tween 实现.
   - **1 `fade_out_and_free()` 0.05s 退出 (T173 #92 落地) 0 重写** — 5 verb 各自 `fade_out_and_free()` 是 byte-identical copy (T166 #85 + T167 #86 + T168 #86 + T169 #87 + T171 #89 落地时各写一遍), 后由 base 1:1 集中: `if not _active: queue_free(); return` 守卫 (idempotent) + `_active = false` 停止 `_process` 与 tween 竞争 + `create_tween()` + `tween_property(self, "modulate:a", 0.0, 0.05)` + `tween_callback(queue_free)`. 由 parent ability 的 `_exit_tree()` 在 scene change / death 时统一调用 (T173 #92 设计本意: 0.05s 短于 room transition ≥0.4s, 长于 abrupt _exit_tree 平滑). 5 verb 子类**必须** 0 override `fade_out_and_free()`. **Whisper 0 extend base 漂移** — `whisper_windup_vfx.gd fade_out_and_free` 自实现 (用 `is_inside_tree()` 守卫 + `tween_property modulate:a 0.0 0.05` + `tween_callback queue_free`, 0 触碰 base 实现, 但功能等价) — 6 verb (Whisper) T275+ 重构目标.
4. **预防** — 任何 polish 期给 6 verb windup VFX 子类 (Pulse / Bind / Cut / Echo / Wave / **Whisper**) 加新 1 共享契约字段 (e.g. `_shared_windup_color`) 或方法 (e.g. `_shared_windup_init`) 时**必须**严格按 1:1 共享契约复制既有模式, 0 漏 0 改 1 字符. 0 重声明 base 3 字段 (`_max_lifetime` / `_lifetime` / `_active`), 0 override `_ready()` (z_index=10), 0 override `_process(delta)` (lifetime + safety net), 0 override `_activate_windup_tween()` (ramp-in tween), 0 override `fade_out_and_free()` (0.05s 退出). 0 触碰 5 verb 各自 `trigger()` 末尾的 `_activate_windup_tween()` 1 行调用 (T174.B #94 替代 5 verb 各自 4-line tween block, 0 触碰). **Whisper 已知 drift** — F013.E (#159) 落地时 0 extend base, 自实现字段 (`_is_active` vs base `_active` 命名偏离) + 自实现 `_process(delta)` + 自实现 `fade_out_and_free()`, 0 触碰 base, 0 触碰 5 verb; T275+ 重构目标: 让 Whisper 也 `extends "res://src/scripts/_verb_windup_vfx_base.gd"` + 删 3 字段 + 删 `_process` + 删 `fade_out_and_free` + 字段名 `_is_active` → `_active` 1:1 rename. T274 (#193) **不** 触碰 Whisper 任何代码 (0 副作用硬约束), 仅在 §9.6.19 文档化该 drift + 下一轮 T275+ 重构目标.

**7 件共享契约 1:1 严格分离总表** (T174.B #94 落地后, 5 verb windup VFX 子类 extends VerbWindupVFXBase 共享契约 0 互混 0 复用 0 共享, 3 字段 + 1 `_ready()` + 1 `_process(delta)` + 1 `_activate_windup_tween()` + 1 `fade_out_and_free()` = 7 件 + 守卫, 0 触碰 0 重写 0 重声明, 1 verb Whisper **当前 0 extend** 是已知 drift):

| 共享契约 | 数量 | base 落地 | 5 verb 子类复制 | Whisper 漂移 (F013.E #159) | 0 触碰边界 |
|------|------|------|------|------|------|
| 共享字段 | 3 (`_max_lifetime` / `_lifetime` / `_active`) | T166 #85 + T173 #92 | 0 重声明 (会 shadow) | **0 extend**, 自实现 `_lifetime` / `_max_lifetime` / `_is_active` (命名偏离) | 5 verb 0 改 0 删 0 重排 |
| `_ready()` | 1 (`z_index = 10`) | T174.B #94 | 0 override | 自实现 `z_index = 50` (VFX 在玩家上方, 5 verb `z_index = 10` 在世界上方) | 5 verb 0 override |
| `_process(delta)` | 1 (lifetime + safety net + `queue_redraw()`) | T174.B #94 | 0 override | 自实现 (调 `fade_out_and_free()` 替代 `queue_free()`) | 5 verb 0 override |
| `_activate_windup_tween()` | 1 (T174 #93 ramp-in modulate:a 0.0→1.0 over _max_lifetime, TRANS_QUAD EASE_OUT) | T174 #93 | 0 override, `trigger()` 末尾 1 行调用 | N/A (Whisper 0 extend, 自实现 trigger) | 5 verb 0 override |
| `fade_out_and_free()` | 1 (T173 #92 0.05s modulate:a 1.0→0.0 + `tween_callback(queue_free)`, idempotent) | T173 #92 + T174.B #94 | 0 override | 自实现 (用 `is_inside_tree()` 守卫 + 等价 modulate:a 0.0 0.05 tween, 0 触碰 base) | 5 verb 0 override |
| `super._ready()` 首行 | 1 (base 1 行 z_index=10 集中) | T174.B #94 隐式 | 5 verb 0 override `_ready` → super 自动调用 | N/A (Whisper 0 extend) | 5 verb 0 漏 |
| `extends VerbWindupVFXBase` 1:1 | 1 (T174.B #94 5 verb 0 重写 base 实现) | T174.B #94 | 5 verb 全 `extends "res://src/scripts/_verb_windup_vfx_base.gd"` | **0 extend base** (F013.E #159 落地时漂移) | 5 verb 0 改 `extends` |

**Whisper 已知 drift 详细 (F013.E #159 落地时漂移, T275+ 重构目标)**:

| 漂移项 | Whisper 当前实现 | base 5 verb 实现 | 影响 | T275+ 重构方向 |
|------|------|------|------|------|
| `extends` | `extends Node2D` | `extends "res://src/scripts/_verb_windup_vfx_base.gd"` | 0 共享 7 件契约 | 改 `extends` |
| 字段 `_active` vs `_is_active` | `var _is_active: bool = true` | `var _active: bool = false` | 命名偏离 1 字段 | rename `_is_active` → `_active` |
| `_ready()` z_index | `z_index = 50` (VFX 在玩家上方) | `z_index = 10` (VFX 在世界上方) | z_index 业务语义不同 (Whisper 5 verb 视觉组连贯需在玩家上, 5 verb windup 在世界上) | 保留 0 改 (业务需求) |
| `_process(delta)` 退出路径 | `if _lifetime >= _max_lifetime: fade_out_and_free()` | `if _lifetime >= _max_lifetime: _active = false; queue_free()` | Whisper 退出走 tween 0.05s, 5 verb 走 immediate queue_free (safety net) | T275+ 决定: 统一走 fade_out_and_free (5 verb safety net 改 tween) 或保留 5 verb safety net (Whisper 改 queue_free) |
| `fade_out_and_free()` 守卫 | `is_inside_tree()` 守卫 + `_is_active = false` + tween | `_active` 守卫 + `_active = false` + tween | Whisper 守卫更严格 (is_inside_tree 二次保险) | 0 改 base 守卫 (保留 5 verb 1:1) |

**与 §9.6.18 关系** — §9.6.18 覆盖 `_verb_ability_base.gd` 6 verb ability 子类共享契约 (6 字段 + 1 `@onready` + 2 `@export` + 5 共享方法 + 1 `_exit_tree` 兜底 + 1 `is_globally_blocking` 扩展 = 16 件 1:1 严格分离), §9.6.19 覆盖 `_verb_windup_vfx_base.gd` 5 verb windup VFX 子类共享契约 (3 字段 + 1 `_ready()` + 1 `_process(delta)` + 1 `_activate_windup_tween()` + 1 `fade_out_and_free()` = 7 件 1:1 严格分离). §9.6.18 关注"verb ability 类层级共享契约" (PulseAbility / BindAbility / ...), §9.6.19 关注"windup VFX 类层级共享契约" (PulseWindupVFX / BindWindupVFX / ...). 任何"加新 1 verb"必须**先**按 §9.6.18 在 `_verb_ability_base.gd` 加 1 个 verb ability 子类 extends (0 改 base 既有 6 字段), **同时**按 §9.6.19 在 `_verb_windup_vfx_base.gd` 加 1 个 windup VFX 子类 extends (0 改 base 既有 7 件共享契约, 0 改 1 verb Whisper 已知 drift 任何代码 — T275+ 收回), **再**按 §9.6.16 在 player.gd 加 5 件套 hit handler, **最后**按 §9.6.17 在 player.gd `_ready` 加 5+1 件套 signal 桥接. 4 套 polish 模式 0 反序 0 漏 0 改 1 字符. §9.6.18 + §9.6.19 是 ability + VFX **双 base 1:1 严格分离**镜像, 6 verb 子类 (ability) + 6 verb 子类 (windup VFX) **双 6 件 1:1 严格分离 = 12 件 1:1 严格分离 0 互混 0 复用 0 共享**.

**与 §9.6.16 / §9.6.17 / §9.6.18 / §9.1 6 verb 接入路径 9 步关系** — §9.1 9 步 = `verb` (1) + `ability` (2) + `const` (3) + `input` (4) + `shop` (5) + `icon` (6) + `vfx` (7) + `audio` (8) + `HUD 冷光勾边` (9). §9.6.19 共享契约走"vfx (7)" 步 (`src/scripts/_verb_windup_vfx_base.gd` 共享契约 1:1 严格分离) + §9.1 9 步落地的"verb windup VFX 子类 extends" 链 (per verb `extends "res://src/scripts/_verb_windup_vfx_base.gd"`). 任何"加新 1 verb"必须**先**按 §9.1 9 步落地, **再**按 §9.6.19 共享契约 1:1 复制 (0 反序 0 漏 0 改 1 字符). 0 触碰 §9.1 9 步任何 const / var / 节点 (共享契约仅在 base `_verb_windup_vfx_base.gd`, 5 verb 子类 0 改 base 0 改, 1 verb Whisper T275+ 收回).
