...
# frame jingle + windup tick all owned by VerbAbilityBase).  Pulse
# has no verb-specific _on_extra_process work, so the base default
# no-op is correct.
func _process(delta: float) -> void:
	super(delta)

func can_pulse() -> bool:
	return _cooldown_timer <= 0 and GameState.resonance >= pulse_cost and not _is_winding_up and _can_fire_extra()
...
# D002.B (#98) — _on_windup_expired (was _execute_pulse) — verb-specific
# fire logic.  Calls _execute_verb_common() for shared bookkeeping.
func _on_windup_expired() -> void:
	# Common bookkeeping: _is_winding_up=false, _cooldown_timer=cooldown, record_ability_used
	...
	# (pulse_vfx.gd's expanding Coral ring).  The audio stream is read
	# from AudioManagerEnhanced._pulse_stream (lazy-allocated by the
	# manager itself).  Guarded by is_instance_valid on _player so an
