...
下一轮（#87，N%5=2，普通模式）建议候选（VFX polish + 文档收尾 + Phase 3 评估延续）：
- T164 [候选] Polish InkWarden phase 3 dissolve 0.20s 出 + 0.25s 入 tween（**#85 #86 因 phase 3 锚定不明延后**，下次评估"phase 3" 究竟是 _break_shield / _enter_stun / _purify 三选一）(10min)
- T169 [候选] Polish CutAbility 起手 0.06s 黄色 line streak pre-cut 视觉信号（与 T166/T167/T168 一致节奏统一 4 verb windup 语言，目前 Cut 走 0.04s instant-fire）(10min)
- F007 [候选] Tech debt: 提取 4 verb 的 `start_pulse/start_bind/start_cut/start_echo` 公共 "ability-null + cost + cooldown 守卫" 整段到 `_try_verb_ability()` helper（F006 进阶版，进 `ability` 内部而非 `player` 外部）(25min)
- I005 [候选] Info 视情况添加 `tools/test_t167_t168_f006_smoke.gd` 冒烟测试（仿 #84 / #85 模板，~200 行覆盖 3 任务 18+ 项断言），仅在后续 #87 视情况添加
