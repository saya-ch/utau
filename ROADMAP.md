...
下一轮（#89，N%5=4，普通模式）建议候选（Polish + Refactor + Tech debt 路线延续）：
- F007b [候选] Tech debt 真版 base class 抽取：新建 `src/scripts/_verb_ability_base.gd` 30 行 base class（字段：_is_winding_up / _windup_timer / _pending_origin / _pending_direction / _windup_vfx / 方法：_consume_verb_cost / _setup_windup_state / _free_windup_vfx / _exit_tree） + 4 verb `extends "res://src/scripts/_verb_ability_base.gd"` + 删除 2 helper 重复（**风险**：@onready 字段顺序与 _ready() 主动逻辑继承链需谨慎处理）(30min)
- T164 [候选] Polish InkWarden "phase 3" dissolve 0.20s 出 + 0.25s 入 tween（**#85 #86 #87 #88 因 phase 3 锚定不明连续延后** —— 当前 ink_warden.gd 候选 "phase 3" 究竟是 _break_shield / _enter_stun / _purify 三选一，**先确定锚点**再设计 dissolve 路径）(10min)
- T170d [候选] Polish Cut 命中屏抖（**T170 余下 1 verb 命中反馈**——T170a/b/c 在 #88 落地了 Bind 命中 / Echo 命中非反弹 / Pulse 命中屏抖 3 反馈补缺，**Cut 命中屏抖**未做；`_on_cut_hit` 已有 Amber flash (T098) + _on_cut_fired shake (T089)，**Cut 命中反馈是否需要再补 LIGHT shake** 是 design 决定先评估边际价值）(10min)
- F008 [候选] Tech debt: 4 verb 内部 `_setup_windup_state` 改用 shared base class（被 F007b 包含，独立候选下可单独做）(15min)
- I006 [候选] Info 视情况补 #88 缺测试覆盖率：T170a/b/c 3 任务有 21 项锚点断言但**没有运行时实例化测试**（需要 mock ScreenShake autoload，**谨慎处理 mock 边界**）(15min)
