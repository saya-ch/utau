...
下一轮（#85，N%5≠0，普通模式）建议候选（polish 路线延续）：
- T164 [候选] Polish InkWarden phase 3 dissolve 0.20s 出 + 0.25s 入 tween（T159 phase 2 同模式，phase 3 也应统一）(10min)
- T165 [候选] Polish BGM tier-up visual cue (audio_manager_enhanced._on_tier_up 链入 brief 0.15s Glass Cyan 闪) (10min)
- T166 [候选] Polish PulseAbility windup 0.10s 加 0.5× 圆环 pre-pulse（pre-emptive 视觉信号让玩家更易预判）(15min)
- F005 [候选] Tech debt: 提取 `player.gd` `_handle_pulse` / `_handle_bind` / `_handle_cut` / `_handle_echo` 公共 OR 守卫到 `_pre_verb_block_check()` helper (减少 4 处 `is_action_globally_blocked` 重复) (15min)
