...
**5 verb 音频闭环现状**（#94 F004 + #96 F004.B + #97 T181）：Pulse fire（F004）+ Bind/Cut/Echo/Wave fire 4 SFX（F004.B）全部 `AudioManagerEnhanced.play_*()` 定义完成 + 5 verb ability `_execute_*()` 全部接入 fire caller (#97 T181) + 5 verb cooldown ready jingle `play_*_cooldown_ready()` (#97 T181) + `_cooldown_was_active` edge detect 防 ready 期间重复触发 (#97 T181)。剩余 T181.B 候选闭环：5 verb hit audio（Pulse fire 复用, Wave T141 已有, 剩 Bind/Cut/Echo 三 verb hit）。**5 verb ability 共享基类**（#97 D002.B）：[`src/scripts/verb_ability_base.gd`](file:///workspace/src/scripts/verb_ability_base.gd) 新基类 `class_name VerbAbilityBase extends Node` 含 2 个共享 helper (`_has_audio_manager_enhanced()` + `_has_game_state_autoload()`)，5 verb 全部 `extends VerbAbilityBase`（5 verb 删本地 9 份 byte-identical 复制粘贴 → 1 份共享基类，净 -20 LOC + future-proof 1 处改 vs N 处改），与 #94 T174.B `VerbWindupVFXBase` 形成姊妹经验（5 verb windup VFX 家族 + 5 verb ability 家族各共享 1 父类）。
...
- **#97 — T181 5 verb 音频家族 caller + cooldown ready 闭环 + D002.B 5 verb ability _VerbAbilityBase 共享基类抽取 (本轮)**：[`src/scripts/audio_manager_enhanced.gd`](file:///workspace/src/scripts/audio_manager_enhanced.gd) 新增 5 verb cooldown ready jingle 公开 API（`play_pulse_cooldown_ready()` / `play_bind_cooldown_ready()` / `play_cut_cooldown_ready()` / `play_echo_cooldown_ready()` / `play_wave_cooldown_ready()`）+ 5 个 `_pulse_cooldown_stream` / `_bind_cooldown_stream` / `_cut_cooldown_stream` / `_echo_cooldown_stream` / `_wave_cooldown_stream` AudioStreamWAV 缓存字段（lazy-init 模式与 F004.B #96 4 verb fire SFX 模板同构）+ 私有 `_generate_cooldown_sfx(base_freq: float)` 工厂函数（sample_rate=44100, 0.10s 短促, 5 verb 主题频率 Pulse 220Hz / Bind 150Hz / Cut 1500Hz / Echo 1320Hz / Wave 200Hz, exp(-t*20.0) 急 decay, amplitude 0.18 比 fire SFX 0.28-0.35 轻 50% 表达"UI 提示"而非"战斗事件"）。**5 verb ability `_execute_*()` 接入 fire caller**（4 verb 新增 — Pulse #94 F004 已有）— [`bind_ability.gd`](file:///workspace/src/scripts/bind_ability.gd) `_execute_bind` / [`cut_ability.gd`](file:///workspace/src/scripts/cut_ability.gd) `_execute_cut` / [`echo_ability.gd`](file:///workspace/src/scripts/echo_ability.gd) `_execute_echo` / [`resonance_wave_ability.gd`](file:///workspace/src/scripts/resonance_wave_ability.gd) `_execute_wave` 全部调 `AudioManagerEnhanced.play_*()` + 8 处 T181 (#97) attribution docblock（fire-VFX 视觉 × fire 音色配对，4 verb 全部 `if _player and is_instance_valid(_player):` 守卫防 windup 中断 stale ref 崩溃）。**5 verb `_cooldown_was_active` edge detect 防 ready 期间重复触发**：5 verb ability 全部新增 `var _cooldown_was_active: bool = false` 字段 + `_process(delta)` 加 edge detect 块（`_cooldown_timer -= delta` 之后 `if _cooldown_timer <= 0 and _cooldown_was_active: _cooldown_was_active = false; if _has_audio_manager_enhanced(): AudioManagerEnhanced.play_*_cooldown_ready()`）；`_execute_*` 开头设 `_cooldown_was_active = true`（F142 #75 5-verb 链防误触同模式：mark active 在 fire 时刻, edge-detect 在 cooldown 完成时刻, fire 一次后清零避免 cooldown 期间每帧重放 ready jingle）。**D002.B 5 verb ability 共享基类** — 新建 [`src/scripts/verb_ability_base.gd`](file:///workspace/src/scripts/verb_ability_base.gd) 父类（`class_name VerbAbilityBase extends Node` 60 行，2 个共享 helper `_has_audio_manager_enhanced()` + `_has_game_state_autoload()` 抽到 base, 字节级与原 5 verb 内 9 份 byte-identical 复制粘贴保持一致）；5 verb ability 全部 `extends VerbAbilityBase`（5 verb 删本地 9 份副本，净 -20 LOC, future-proof 1 处改 vs N 处改），与 #94 T174.B `VerbWindupVFXBase` 形成姊妹经验（5 verb windup VFX 家族 + 5 verb ability 家族各共享 1 父类）。**I011 29 项锚点 smoke 测试**（[test_i011_t181_d002b_smoke.gd](file:///workspace/tools/test_i011_t181_d002b_smoke.gd) 新文件 260 行）**29/29 PASS**：T181 19 项（5 verb cooldown ready 公开 API × 5 + _generate_cooldown_sfx 工厂签名 × 1 + 5 verb 缓存字段 × 5 + 4 verb fire caller 接入 × 4 + 5 verb _cooldown_was_active 字段 × 5 + 5 verb _process edge detect × 5 + 4 verb _execute_* 设 _cooldown_was_active=true × 4）+ D002.B 16 项（class_name VerbAbilityBase × 1 + extends Node × 1 + 2 shared helper × 2 + 5 verb extends VerbAbilityBase × 5 + 5 verb 删 _has_audio_manager_enhanced 副本 × 5 + 2 verb 删 _has_game_state_autoload 副本 × 2）。**45/45 smoke test 套件 100% PASS**（#96 44 + I011 +1 = 45，0 回归, I010 #96 56 项 + I009 #94 50 项 + I008 #93 40 项 关键 anchor 无回归）+ `check_smoke_consistency.sh` 7/7 规则 PASS + 0 SCRIPT ERROR（静态层 5 verb 删本地 helper 计数 0 + base helper 2 + 5 verb extends base + 1 base class_name 6 匹配全部就位）。**5 verb 音频家族 5/5 → 10/12 cue 闭合**（fire 5/5 + cooldown 5/5 完成, 剩 hit 5 cue 待 #98 T181.B）。
