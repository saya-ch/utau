...
- **#79 — T152 启动 run-trend cache 预热 + T153 run-history 灰度叠加 + T151 save_focus 缩短间隔**：T152 `SaveSystem._refresh_run_trend_cache()` 在每次 save + load 路径预热（缓存每次 save 都"保鲜"，首次查询 O(1)）；T153 `pause_menu.gd` history card 槽位用 `Color(0.4,0.4,0.4,0.4)` 灰度色覆盖当 `card.get_modulate().a < 0.8`（recent=1.0 完整 / older=0.6 灰度叠加）；T151 `SaveSystem` 新增 `save_focus_interval` (默认 0，关闭；可设 600s 短间隔) 与 `save_focus` perk 协同（玩家购买后 `set_save_focus_interval(600)` 主动缩窗）；`test_t152_t153_t151_smoke.gd` 10 项断言 PASS
- **#78 — T144 wave_focus 谐波 + T148 wave_combo chime tail + T154 灯反向闪**：T144 `ShopMenu` 升级 `wave_focus` perk → wave_hit_sfx 链 1.06 → 1.12 / hit_count 1.0 → 1.1 (chime tail 加 6% 谐波, 2.4x/3.0x/3.6x → 2.55x/3.18x/3.82x); T148 `resonance_wave_ability.gd._on_wave_combo()` 2.0x→0.65x chime tail 渐降 0.4s 替代硬切（更自然"能量消散"）；T154 `save_lantern.gd` 反向闪：玩家拾取后 lantern 自身 light 0.0 → 1.4 渐亮 0.4s 替代被压暗的"灯无反应"；`test_t144_t148_t154_smoke.gd` 11 项断言 PASS
- **#77 — T150 5 动词 profile 对齐 + T147 jump 阻塞 UX + T149 Echo parallax 双层**：T150 `pause_menu.gd._refresh_profile()` 加 `abilities_total: int = 5` 字段让 Player Profile 卡片同时显示 4 动词已用 + 5 动词 total（与 HUD WaveRow 第五冷却条对称）；T147 `player.gd._handle_jump()` 加 `is_action_globally_blocked()` 守卫——`is_dashing` / `is_purified` / `is_winding_wave` / `is_dead` 全部拒绝跳（消除"`_try_fire` 失败但角色已起跳"的视觉错位）；T149 `echo_vfx.gd._process()` 双层 parallax（主层 position `+= base_drift * dt` + 副层 `position.x = base_x + 0.5 * sin(t * 0.3) * 8.0` 独立水平波动）；`test_t150_t147_t149_smoke.gd` 13 项断言 PASS
- **#76 — T143 wave 4 状态提示路由 + T145 is_action_globally_blocked 重构 + T146 wave_combo 屏震**：T143 `_handle_wave` 失败提示路由 (silenced → `_wave_silenced_prompt` / off_cooldown → `_wave_off_cooldown_prompt` / already_active → `_wave_already_active_prompt` / no_resource → 通用) 与 _handle_bind 对称；T145 `player.gd` 把 #75 T141 `_is_wave_globally_blocking()` 升级为 `is_action_globally_blocked()`（涵盖 wave windup / purify / dead / dash 全场景，所有 verb input 统一守卫）；T146 `resonance_wave_ability.gd._on_wave_combo` ScreenShake `kick(Electric Violet 8C5BFF, 0.4 * hit_count, 0.06)` 屏震（按命中数等比缩放）；`test_t143_t145_t146_smoke.gd` 10 项断言 PASS
